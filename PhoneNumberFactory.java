...
public final class PhoneNumberFactory {
    ...
    public static PhoneNumber[] createRandomPhoneNumberArray(int phoneNumberCount) {
        ...
        for (int s = 0; s < phoneNumberCount; s++) {
            numbers[s] = createRandomPhoneNumber();
        }
        return numbers;
    }
...
//        (###)-###-####
        String v = "(" + areaCode + ")" + "-" + centralOfficeCode + "-" + phoneLineCode;
        try {
            return createPhoneNumber(v);
        } catch (InvalidPhoneNumberFormatException e) {
           e.printStackTrace();
            logger.info(v + " is not a valid phone number");
